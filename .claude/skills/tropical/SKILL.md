...
description: >-
  ...
  world behind the UI. The words "TROPICAL", "טרופי", "אי", "ג'ונגל",
  "בסגנון האי" or "בסגנון הטרופי" anywhere in a request ARE the trigger - they
  mean "build this looking like the island theme". The design questions are
  already answered here, so apply it instead of asking how it should look. Use
  it whenever building, restyling or reviewing ANY interface in this theme - a
  screen, modal, button, card, list row, leaderboard, badge, avatar ring,
  toast, board skin, arena or mockup. Consult it before writing any CSS or HTML
  so new UI reuses these tokens instead of inventing colors, radii and bevels
...
direction from this file and get on with the work; don't ask them to describe the
look again, and don't offer a menu of alternatives.
...
system, `showScreen()`, the component class names, the icon pipeline, the RTL and
...
They are the source of truth; this file is the map. If a token below disagrees with
the CSS, the CSS wins and this file should be corrected.
...
Gold fill, thick brown outline, hard drop shadow — the "sticker" look of the logo and
the `יניב!` speech bubbles:
...
the panel, not text inside it: `--wood` fill, `--wood-dark` ring, `--radius-pill`,
carved cream text, pulled up with a negative top margin so it straddles the frame.

**Round icon button** — `--gold` face, `--radius-pill`, 1:1, a brown glyph inside,
`--bevel-sm`. This is the help/settings/friends cluster. Icon-only always means an
`aria-label`.
