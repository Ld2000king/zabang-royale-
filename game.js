...
// carries its character (tagline + motif) plus its own board skin, in the
// island palette: a bright tile face, the darker base that forms its bevel,
// and a soft accent that re-tints the grassy table behind the tiles (see
// applyBoardTheme). textLight flips tile text to white where the face is too
// deep for the default brown ink.
const ARENAS = [
...
// Applies an arena skin to a board container by writing the city's tile face,
// its bevel base and a soft accent tint into inline CSS vars (see
// .arena-themed in game.css). renderBoard only clears innerHTML, so these
